...
class SessionManager:
    ...
    def __init__(self, config_file: str = "data/sessions/session.json", backup_count: int = 3):
        """
    ...
    def _load_config(self) -> Dict[str, Any]:
        ...
        try:
            if self.config_file.exists():
                ...
                # Restore Qt objects from JSON
                loaded_config = self._restore_from_json(loaded_config)
    ...
    def _prepare_for_json(self, obj):
        """Convert Qt objects to JSON-serializable format - FIXED"""
        if obj is None:
            return None
            ...
            try:
                ...
                logger.warning(f"Failed to convert QByteArray to JSON: {e}")
                return None
            ...
            try:
                ...
                logger.warning(f"Failed to convert bytes to JSON: {e}")
                return None
        elif isinstance(obj, dict):
            result = {}
            for k, v in obj.items():
                try:
                    result[str(k)] = self._prepare_for_json(v)
                except Exception as e:
                    logger.warning(f"Failed to convert dict key {k}: {e}")
            ...
            for item in obj:
                try:
                    result.append(self._prepare_for_json(item))
                except Exception as e:
                    logger.warning(f"Failed to convert list item: {e}")
            ...
            try:
                ...
                logger.warning(f"Failed to convert object {type(obj)} to JSON: {e}")
                return None

    def _restore_from_json(self, obj):
        """Convert JSON data back to Qt objects - FIXED"""
        if obj is None:
            return None
        elif isinstance(obj, dict):
            if obj.get('_qt_type') == 'QByteArray':
                try:
                    data = base64.b64decode(obj['data'].encode('utf-8'))
                    return QByteArray(data)
                except Exception as e:
                    logger.warning(f"Failed to restore QByteArray: {e}")
                    return None
            elif obj.get('_qt_type') == 'bytes':
                try:
                    return base64.b64decode(obj['data'].encode('utf-8'))
                except Exception as e:
                    logger.warning(f"Failed to restore bytes: {e}")
                    return None
            else:
                result = {}
                for k, v in obj.items():
                    try:
                        ...
                        logger.warning(f"Failed to restore dict key {k}: {e}")
                        # Skip problematic keys
                        continue
                return result
            ...
            for item in obj:
                try:
                    result.append(self._restore_from_json(item))
                except Exception as e:
                    logger.warning(f"Failed to restore list item: {e}")
                    # Skip problematic items
                    continue
            return result
        else:
    ...
    def save_config(self):
        """Save current configuration to file with Qt object handling - FIXED."""
        try:
            # Ensure the parent directory exists
            self.config_file.parent.mkdir(parents=True, exist_ok=True)

            # Convert Qt objects to JSON-serializable format
            json_ready_config = self._prepare_for_json(self.config)
            ...
            with open(self.config_file, 'w', encoding='utf-8') as f:
                json.dump(json_ready_config, f, indent=2, ensure_ascii=False)

            logger.debug(f"Configuration saved to {self.config_file}")

        except Exception as e:
            logger.error(f"Error saving configuration: {e}")
            # Don't re-raise the exception to prevent app crashes
            logger.warning("Continuing without saving session state")
    ...
    def set(self, key: str, value: Any, auto_save: bool = True):
        """Set a configuration value - ENHANCED with error handling."""
        try:
            ...
            # Store the value anyway, just don't save
            self.config[key] = value

    def update(self, updates: Dict[str, Any], auto_save: bool = True):
        """Update multiple configuration values - ENHANCED with error handling."""
        try:
            ...
            # Update the values anyway, just don't save
            self.config.update(updates)
